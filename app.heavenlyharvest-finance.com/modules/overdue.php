...
try {
    ...
    // Fetch all pending/partially-paid overdue instalments where days_overdue < 30
    $query = "
        SELECT 
            li.instalment_id,
            li.loan_id,
        ...
        ORDER BY li.days_overdue DESC, c.customer_name ASC
    ";
    ...
    // Attach computed _days_diff
    foreach ($rows as $n) {
        $due = new DateTime($n['due_date']);
        $due->setTime(0, 0, 0);
        $diff = $today->diff($due);
        $n['_days_diff'] = -((int)$diff->days);
        $overdue[] = $n;
    }

} catch (Exception $e) {
    $error_message = "Error: " . $e->getMessage();
    error_log("overdue.php error: " . $e->getMessage());
}

$total_count = count($overdue);
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Overdue Instalments — Under 30 Days</title>
    ...
    <style>
        ...
        *, *::before, *::after { box-sizing: border-box; }
        ...
        /* ─── HERO STRIP ─── */
        .hero-strip {
            background: linear-gradient(100deg, var(--danger) 0%, #7b1a12 100%);
        ...
        .stat-pill.pill-danger .pill-dot { background: var(--danger); }
        .stat-pill.pill-danger .pill-count { color: var(--danger); }
        ...
        .lbl-danger .label-pill { background: var(--danger-lt); color: var(--danger); }
        ...
        .notif-balance { font-size: 1.15rem; font-weight: 700; letter-spacing: -.5px; color: var(--danger); }
...
<div class="topbar">
    ...
    <div class="topbar-actions">
        <a href="notifications.php" class="btn-nav">
...
<div class="hero-strip">
    <div class="hero-title">
        <i class="fas fa-exclamation-triangle" style="font-size:1.5rem;opacity:.8;margin-right:10px;"></i>
        Overdue Instalments
    </div>
    <p class="hero-sub">Past-due instalments under 30 days overdue — immediate follow-up required</p>
    <p class="hero-date">
...
<div class="content-area">
    ...
    <div class="empty-state">
        ...
        <h5>No Overdue Instalments!</h5>
        <p>There are currently no overdue instalments under 30 days.<br>Check back later.</p>
    </div>
    <?php endif; ?>

    <?php if (!empty($overdue)): ?>
    <div class="section-label lbl-danger">
        <div class="label-pill">
            <i class="fas fa-exclamation-triangle me-1"></i>Overdue — Under 30 Days (<?php echo count($overdue); ?>)
        </div>
        <div class="label-line"></div>
    </div>

    <?php foreach ($overdue as $n):
        ...
        $daysOver  = abs($n['_days_diff']);
        $label     = $daysOver === 1 ? '1 Day Overdue' : $daysOver . ' Days Overdue';
    ?>
    <a href="index.php?page=recordpayment&loan_id=<?php echo $loanId; ?>"
       class="notif-card notif-searchable"
       title="Click to process payment — Loan ID: <?php echo $loanId; ?>">
        <div class="notif-icon"><i class="fas fa-exclamation-triangle"></i></div>
        <div class="notif-body">
            <div class="notif-customer"><?php echo $customer; ?></div>
            <div class="notif-meta">
                <span><i class="fas fa-hashtag"></i> Loan <?php echo $loanNum; ?></span>
                <span><i class="fas fa-layer-group"></i> Instalment #<?php echo $instNum; ?></span>
                <span><i class="fas fa-calendar"></i> Due <?php echo $dueDate; ?></span>
                <span title="Principal / Interest / Fee">
                    <i class="fas fa-coins"></i> P: <?php echo $principal; ?> &nbsp;|&nbsp;
                    I: <?php echo $interest; ?> &nbsp;|&nbsp; F: <?php echo $fee; ?>
                </span>
                <span><i class="fas fa-tag"></i> <?php echo $status; ?></span>
            </div>
        </div>
        <div class="notif-amounts">
            <div class="notif-balance"><?php echo $balance; ?></div>
            <div class="notif-total">Total due: <?php echo $total; ?></div>
            <div class="notif-badge"><?php echo $label; ?></div>
        </div>
